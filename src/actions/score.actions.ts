...
export async function calculateEnterpriseScore(
  ...
  // Calculer les scores individuels pour chaque année (pour comparaison)
  const scoresParAnnee: YearScore[] = [];

  // Score année N (sans évolution)
  const ratiosN = calculateRatios(donneesN);
  scoresParAnnee.push({
    annee: anneeN,
    score: calculateScore(ratiosN, valeursN),
  ...
  if (saveToHistory) {
    savedToHistory = await saveOrUpdateScoreHistory(
      supabase,
      enterpriseId,
      anneeN,
      score
