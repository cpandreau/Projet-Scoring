...
function FormattedValue({ value, unite }: { value: number | null; unite?: string }) {
  ...
  return (
    <span className="font-mono font-semibold">
      {formatted}
      {unite && <span className="ml-1 text-muted-foreground">{unite}</span>}
    </span>
  )
}
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <CollapsibleTrigger asChild>
        <div className="flex cursor-pointer items-center justify-between rounded-lg bg-muted/50 p-3 transition-colors hover:bg-muted">
      ...
      <CollapsibleContent>
        <div className="mt-2 ml-6 space-y-3 rounded-lg border p-3 text-sm">
          <div>
            <span className="text-muted-foreground">Formule : </span>
            <span className="rounded bg-muted px-2 py-1 font-mono text-xs">{detail.formule}</span>
          </div>
          ...
          <div>
            ...
            <div className="mt-1 rounded bg-green-50 px-2 py-1 font-mono text-green-800 text-xs dark:bg-green-950 dark:text-green-200">
              {detail.calcul}
            </div>
          </div>
        </div>
...
function RatioCard({ name, detail }: { name: string; detail: RatioDetail }) {
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      ...
      <CollapsibleContent>
        <div className="mt-2 ml-6 space-y-3 rounded-lg bg-muted/30 p-3 text-sm">
          <div>
            <span className="text-muted-foreground">Formule : </span>
            <span className="rounded bg-muted px-2 py-1 font-mono text-xs">{detail.formule}</span>
          </div>
          ...
          <div>
            ...
            <div
              className={cn(
                'mt-1 rounded px-2 py-1 font-mono text-xs',
                detail.valeur !== null
...
function RatioSection({ title, ratios }: { title: string; ratios: Record<string, RatioDetail> }) {
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <CollapsibleTrigger asChild>
        <div className="flex cursor-pointer items-center justify-between rounded-lg bg-primary/10 p-2 transition-colors hover:bg-primary/20">
          <div className="flex items-center gap-2">
...
export function RatioDebug({ donnees }: RatioDebugProps) {
  ...
  // useMemo must be called before any conditional return (React hooks rules)
        ...
        <ScrollArea className="h-[70vh] pr-4">
          <div className="space-y-6">
            ...
            <Collapsible defaultOpen={false}>
              <CollapsibleTrigger asChild>
                <div className="flex cursor-pointer items-center justify-between rounded-lg bg-blue-50 p-3 transition-colors hover:bg-blue-100 dark:bg-blue-950 dark:hover:bg-blue-900">
                  <div className="flex items-center gap-2">
                ...
                <div className="mt-2 rounded-lg border p-3">
                  <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
                    {Object.entries(details.donnees).map(([key, value]) => (
                      <div key={key} className="flex justify-between gap-2 rounded bg-muted/30 p-1">
            ...
            <div>
              ...
              <div className="space-y-2">
                {Object.entries(details.aggregats).map(([key, detail]) => (
                  <AggregateCard key={key} name={AGGREGATE_LABELS[key] || key} detail={detail} />
                ))}
              </div>
            </div>
