...
import { type FamilyId, getRatiosByFamily, RATIO_FAMILIES, RATIOS } from '@/config/ratios.config'
import type { ExtractionValues } from '@/schemas/extraction.schema'
import type { CalculatedRatios } from './calculate'
import { type ExcludedRatio, isRatioRelevant } from './relevance'
import { getZone, type QuartilesMap, type Zone } from './zone'
...
export function calculateScore(
  ...
  const familyIds: FamilyId[] = ['liquidite', 'rentabilite', 'solvabilite', 'activite', 'evolution']

  // Calculer le score de chaque famille
  const scoreParFamille: Record<FamilyId, FamilyScore> = {} as Record<FamilyId, FamilyScore>
  ...
  for (const familyId of familyIds) {
    const { familyScore, excluded } = calculateFamilyScore(familyId, ratios, donnees, quartiles)
  ...
  return {
    ...
    statistiques: {
      ...
      tauxCompletude: totalRatios > 0 ? Math.round((ratiosCalculables / totalRatios) * 100) : 0,
    },
  }
}
...
export function generateScoreSummary(result: ScoreResult): string {
  ...
  for (const family of Object.values(result.scoreParFamille)) {
    lines.push(`  • ${family.nom} (${family.poids}%) : ${family.score.toFixed(1)}/10`)
  }
