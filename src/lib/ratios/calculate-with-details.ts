...
import {
  ...
  calculatePassifCirculant,
  calculateCapitauxPermanents,
  calculateFRNG,
  calculateBFR,
  calculateVAFromData,
  calculateEBE,
  calculateCAF,
  calculateMargeCommerciale,
...
// Helper pour construire une chaîne de calcul
function buildCalcString(
  operation: string,
  values: (number | null)[],
  result: number | null
): string {
  const formattedValues = values.map(fmt).join(` ${operation} `);
  return `${formattedValues} = ${fmt(result)}`;
}
...
function calculateLiquiditeDetails(
  ...
  // Liquidité générale
  const liq_gen_ratio =
    passif_circulant && passif_circulant !== 0
      ? (actif_circulant ?? 0) / passif_circulant
      : null;
...
function calculateRentabiliteDetails(
  ...
  const rent_fin_ratio = capitaux_propres && capitaux_propres !== 0 ? (caf ?? 0) / capitaux_propres : null;
...
function calculateSolvabiliteDetails(
  ...
  const cap_remb = caf && caf > 0 && dettes_financieres !== null ? (dettes_financieres / caf) * 360 : null;
...
function calculateActiviteDetails(
  ...
  const rfr_ratio =
    actif_immobilise && actif_immobilise !== 0 ? (capitaux_permanents ?? 0) / actif_immobilise : null;
