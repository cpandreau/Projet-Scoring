...
import { fetchSectorBenchmarks, getTrancheCA, TRANCHES_CA } from './sector-benchmarks'
import { extractQuartiles, RATIO_MAPPINGS } from './sector-benchmarks-mapping'
...
export interface SectorComparisonResult {
  ...
  exerciceEntreprise: number // Annee des ratios de l'entreprise (ex: 2024)
  exerciceSecteur: string // Annee des benchmarks sectoriels (ex: "2022")
  cohorte: number // Nombre d'entreprises dans la cohorte
  anneesDisponibles: number[] // Annees disponibles pour l'entreprise
...
export async function comparerAuSecteur(
  ...
  exerciceEntreprise: number,
  anneesDisponibles: number[],
  anneeSecteur?: string
): Promise<SectorComparisonResult> {
  // Recuperer les benchmarks depuis l'API
  // Si pas d'annee specifiee, on cherche la plus proche de l'exercice entreprise
  const anneeRecherche = anneeSecteur || exerciceEntreprise.toString()
  const benchmark = await fetchSectorBenchmarks(codeNaf, chiffreAffaires, anneeRecherche)
  ...
  return {
    ...
    exerciceEntreprise,
    exerciceSecteur: benchmark.exercice,
