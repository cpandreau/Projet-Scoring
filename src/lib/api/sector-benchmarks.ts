...
const API_BASE_URL = 'https://data.economie.gouv.fr/api/explore/v2.1/catalog/datasets'
...
export async function fetchSectorBenchmarks(
  ...
  try {
    ...
    // Filtre optionnel sur l'annee
    if (annee) {
      conditions.push(`exercice = '${annee}'`)
    }
    ...
    if (!response.ok) {
      ...
      if (!fallbackResponse.ok) {
        console.error('[SectorBenchmarks] Fallback failed:', fallbackResponse.status)
        return null
      }
