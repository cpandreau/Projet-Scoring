...
export async function generateScoreReport(data: ReportData): Promise<Uint8Array> {
  const doc = new jsPDF({
  ...
  // Exercice
  doc.text(`Exercice ${data.anneeExercice} - Rapport du ${data.dateCalcul}`, pageWidth / 2, y, {
    align: 'center',
  })
  y += 25
  ...
  for (const f of familles) {
    ...
    const zoneColor = scoreVal >= 7 ? '#22c55e' : scoreVal >= 5 ? '#f59e0b' : '#ef4444'
  ...
  for (const [famille, ratios] of Object.entries(byFamille)) {
    ...
    for (const ratio of ratios) {
      ...
      const valeur = ratio.valeur !== null ? `${formatNumber(ratio.valeur)} ${ratio.unite}` : 'N/A'
      const zone = ratio.zone === 'vert' ? 'Bon' : ratio.zone === 'jaune' ? 'Moyen' : 'Risque'
      const zoneColor =
        ratio.zone === 'vert' ? '#22c55e' : ratio.zone === 'jaune' ? '#f59e0b' : '#ef4444'
