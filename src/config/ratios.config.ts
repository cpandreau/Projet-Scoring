 ...
 */
export function getScoreFromEvaluation(
  evaluation: "green" | "yellow" | "red"
): number {
  switch (evaluation) {
    case "green":
      return 100;
...
export function validateFamilyWeights(): boolean {
  const totalWeight = Object.values(RATIO_FAMILIES).reduce(
    (sum, family) => sum + family.poids,
    0
  );
...
if (!validateFamilyWeights()) {
  console.warn(
    "Attention: La somme des poids des familles de ratios n'est pas égale à 100%"
  );
}
