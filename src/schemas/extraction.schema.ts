...
export const extractionSchema = z.object({
  // Métadonnées
  type_liasse: typeLiasseSchema.describe(
    "Type de liasse fiscale identifié : 'normale' (CERFA 2050-2059) ou 'simplifiee' (CERFA 2033)"
  ),

  // Compte de résultat
  chiffre_affaires: valueWithSourceSchema.describe(
  ...
  achats_matieres_premieres: valueWithSourceSchema.describe(
  ...
  charges_personnel: valueWithSourceSchema.describe(
  ...
  // Bilan - Actif
  actif_immobilise: valueWithSourceSchema.describe(
    'Total actif immobilisé (immobilisations incorporelles + corporelles + financières)'
  ),
  actif_circulant: valueWithSourceSchema.describe(
    'Total actif circulant (stocks + créances + disponibilités)'
  ),
  stocks: valueWithSourceSchema.describe(
    'Total des stocks (marchandises + matières premières + en-cours + produits finis)'
  ),
  ...
  dettes_financieres: valueWithSourceSchema.describe(
    'Dettes financières (emprunts + dettes bancaires)'
  ),
  dettes_fournisseurs: valueWithSourceSchema.describe('Dettes fournisseurs et comptes rattachés'),
  decouvert_bancaire: valueWithSourceSchema.describe(
  ...
  dettes_fiscales_sociales: valueWithSourceSchema.describe(
    'Dettes fiscales et sociales (personnel, organismes sociaux, État)'
  ),
  comptes_courants_associes: valueWithSourceSchema.describe(
    "Comptes courants créditeurs d'associés"
  ),
  provisions_risques_charges: valueWithSourceSchema.describe('Provisions pour risques et charges'),
  dotations_amortissements: valueWithSourceSchema.describe(
